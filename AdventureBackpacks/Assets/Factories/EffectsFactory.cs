...
public class EffectsFactory : FactoryBase
{
    ...
    public EffectsFactory(ModLogger logger, ConfigRegistry configs) : base(logger, configs)
    {
        Instance = this;
    }
    ...
    {
        if (_externalEffects.Any(x => x.EffectName.Equals(effectDefinition.EffectName, StringComparison.Ordinal))) return;
