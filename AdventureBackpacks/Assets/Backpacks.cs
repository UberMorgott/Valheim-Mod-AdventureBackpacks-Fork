...
namespace AdventureBackpacks.Assets
{
    public static class Backpacks
    {
        ...
        private static ModLogger _log = AdventureBackpacks.Log;
        ...
        internal static bool TryGetBackpackItemByName(string name, out BackpackItem backpack)
        {
            ...
            backpack = BackpackFactory.BackpackItems.FirstOrDefault(x => x.ItemName.Equals(name, StringComparison.Ordinal));
        ...
        public static void UpdateItemDataConfigValues(object sender, EventArgs e)
        {
            ...
            void SearchInventory(List<ItemDrop.ItemData> inventory)
            {
                ...
                foreach (var item in inventory)
                {
                    ...
                    {
                        var currentBackpack = item.GetOrCreateBackpackComponent();
        ...
        public static Inventory NewInventoryInstance(string name, int itemMQuality = 1)
        {
            ...
            var uiInventoryName = $"{name} $adventurebackpacks_level {itemMQuality}";
        ...
        public static bool CheckForInception(Inventory instance, ItemDrop.ItemData item)
        {
            if (instance.IsBackPackInventory() && Player.m_localPlayer != null)
            {
                ...
                if (item.TryGetBackpackItem(out var backpack))
                {
                    ...
                    {
                        case 1:
                            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_no_inception1");
                            break;
                        case 2:
                            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_no_inception2");
                            break;
                        case 5:
                            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_no_inception3");
                            break;
                        case 10:
                            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_no_inception4");
        ...
        public static bool PerformYardSale(Player mLocalPlayer, ItemDrop.ItemData itemData, bool backpackOnly = false, int numberItems = 0)
        {
            ...
            var backpack = itemData.GetBackpackComponent();
            if (backpack == null)
                return true;
            ...
            if (!backpackOnly)
            {
                ...
                Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_no_inception5");
                AdventureBackpacks.PerformYardSale = false;
            }
            else
            {
                Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_thor_saves_contents");
            }
        ...
        public static void UpdateStatusEffects(ItemDrop.ItemData itemData)
        {
            if (itemData == null)
                return;
            ...
            statusEffect.name = $"SE_{backpackName}_{backpackQuality}";

            var defaultStatusName = $"{backpackName} $adventurebackpacks_level {backpackQuality} $adventurebackpacks_effect";


            if (backpack.ShowBackpackStatusEffect.Value)
            {
                statusEffect.m_name = string.IsNullOrEmpty(backpack.CustomStatusEffectName.Value) ? defaultStatusName : backpack.CustomStatusEffectName.Value;
                statusEffect.m_startMessageType = MessageHud.MessageType.TopLeft;
                statusEffect.m_startMessage = $"$adventurebackpacks_useful_backpack";
            }
            ...
            backpack.UpdateStatusEffects(backpackQuality, statusEffect, modifierList, itemData);

            itemData.m_shared.m_maxDurability = 1000f;
            statusEffect.m_mods = modifierList;
            if (backpack.ShowBackpackStatusEffect.Value)
            {
                statusEffect.m_icon = itemData.GetIcon();
            }

            itemData.m_shared.m_equipStatusEffect = statusEffect;
        }
    }
}
