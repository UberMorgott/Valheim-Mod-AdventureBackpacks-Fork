...
internal abstract class BackpackItem : AssetItem, IBackpackItem
{
    ...
    protected BackpackItem(string assetName, string prefabName, string itemName, string configSection = "", bool externalLocalize = false) : base(assetName, prefabName, itemName)
    {
        ...
        _englishSection = ConfigRegistry.ToEnglish(_configSection);
    ...
    internal void SetupLocalization()
    {
        _englishSection = ConfigRegistry.ToEnglish(_configSection);
        _localizedCategory = Localization.m_instance.Localize(_configSection);
        if (_localizedCategory.Equals(_configSection, StringComparison.Ordinal))
            _localizedCategory = Localization.instance.Localize(_configSection);
    }

    private void SetupBackpackDef()
    {
        Recipe.SectionName = _configSection;
        BackpackSize = new();
    ...
    internal static void SetConfig(ConfigRegistry configSync)
    {
        _config = configSync;
    }

    internal static void SetLogger(ModLogger logger)
    {
        _logger = logger;
    }
    ...
    internal abstract void UpdateStatusEffects(int quality, SE_Stats statusEffect, List<HitData.DamageModPair> modifierList, ItemDrop.ItemData itemData);

    internal virtual void RegisterBackpackSize(int quality = 1, int x = 6, int y = 3)
    {
        ConfigEntry<Vector2> newSize = null;
        ConfigRegistry.BindSynced(_englishSection, $"Backpack Size - Level {quality}", new Vector2(x, y),
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Weight Multiplier", defaultValue,
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Backpack Biome", defaultValue,
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Carry Bonus", defaultValue,
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Show Status Effect", defaultShowStatus,
        ...
        ConfigRegistry.BindSynced(_englishSection, "Custom Effect Name", defaultEffectName,
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Speed Modifier", defaultValue,
    ...
    {
        ConfigRegistry.BindSynced(_englishSection, "Prevent freezing/cold?", defaultValue,
