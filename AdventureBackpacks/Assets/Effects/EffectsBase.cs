...
public abstract class EffectsBase
{
    ...
    protected EffectsBase(string effectName, string effectDesc, bool IsItemSetStatusEffect = false)
    {
    ...
    public virtual bool IsEffectActive(Humanoid human)
    {
        if (human is Player player)
        {
            ...
            {
                if ((backpackBiome & enumKeyBit) != BackpackBiomes.None)
                {
    ...
    // Runs after vanilla Player.UpdateEnvStatusEffects (Player.cs:2208) for the local player.
    ...
    public virtual bool CurrentSwitchSetting()
    {
    ...
    public virtual void RegisterEffectConfiguration()
    {
        ...
        ConfigRegistry.BindSynced(_configSection, "Effect Enabled", true,
    ...
    public void RegisterEffectBiomeQuality(BackpackBiomes biome, int defaultQuality = 0)
    {
        ...
        {
            if ((biome & enumKeyBit) != BackpackBiomes.None)
            {
                ConfigEntry<int> configEntry = null;

                ConfigRegistry.BindSynced(_configSection, $"Effective Quality Level: {enumKeyBit.ToString()}",
                    defaultQuality,
