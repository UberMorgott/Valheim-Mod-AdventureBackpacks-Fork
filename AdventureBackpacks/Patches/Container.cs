...
public static class ContainerPatches
{
    ...
    static class ContainerInteractPatch
    {
        ...
        {
            if (__instance.name.Equals("Player(Clone)", StringComparison.Ordinal) || __instance.GetComponent<Player>() != null)
            {
                __result = false;
                return false;
            }
    ...
    // Vanilla Container.CheckForChanges (Container.cs:154) calls Load() whenever the owning ZDO data revision
    // changes, and Save() (Container.cs:470) writes m_inventory into that ZDO. On the player the ZDO is the
    // player's own, so a mod calling Save on the backpack container would round-trip a stale snapshot back
    // into the backpack inventory. Backpack contents live in BackpackComponent/ItemData instead.
    ...
    static class ContainerSavePatch
    {
        ...
        {
            if (__instance != null && (__instance.name.Equals("Player(Clone)", StringComparison.Ordinal) || __instance.GetComponent<Player>() != null))
                return false;
    ...
    static class ContainerAwakePatch
    {
        ...
        {
            if (instance.name.Equals("Player(Clone)", StringComparison.Ordinal) || instance.GetComponent<Player>() != null)
            {
        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator ilGenerator)
        {
            ...
            if (!patchedSuccess)
            {
                AdventureBackpacks.Log.LogError($"Container.Awake Transpiler Failed To Patch");
                Thread.Sleep(5000);
            }
        }
    }
