...
public static class InventoryPatches
{
    ...
    {
        while (ItemsAddedQueue.Count > 0 && DateTime.Now.Subtract(ItemsAddedQueue.Peek().Value).TotalSeconds > 0.5)
        {
            ...
            if (__instance.IsBackPackInventory())
            {
                ...
                if (player.IsBackpackEquipped())
                {
                    ...
                    if (backpack != null)
                    {
                        ...
                        {
                            // The changed instance is not the component's current one: another mod still holds the
                            // player Container's instance, or it belongs to a carried (unequipped) backpack.
                            ...
                            else
                            {
                                ...
                                var matchingBackpack = allItems?.FirstOrDefault(i => i.IsBackpack() && i.GetBackpackComponent()?.GetInventory() == __instance);
                                if (matchingBackpack != null)
                                {
                                    var comp = matchingBackpack.GetBackpackComponent();
    ...
    private static bool RemoveItemPrefix(Inventory __instance, ItemDrop.ItemData item)
    {
        ...
        {
            if (item.IsBackpack())
                _savedBackpackData = item.GetBackpackComponent();
            return true;
        }
        ...
        var backpack = item.GetBackpackComponent();
        if (backpack == null)
            return true;
        ...
        {
            Player.m_localPlayer.Message(MessageHud.MessageType.Center, "$adventurebackpacks_yard_sale_blocked");
            return false;
        }
    ...
    static class AddItemCraftingPatch
    {
        static void Postfix(Inventory __instance, ref ItemDrop.ItemData __result)
        {
            ...
            {
                // Upgrade: the backpack removed by DoCrafting comes back as this new item (InventoryGui.cs:1822/1887).
                if (_savedBackpackData != null && __result != null && __result.m_shared.m_name == _savedBackpackData.Item.m_shared.m_name)
                    BackpackDataStore.MoveToUpgradedItem(_savedBackpackData, __result);
            ...
            static void Postfix(Inventory __instance, ref bool __result, bool allowAllItems)
            {
                ...
                if (__instance == Player.m_localPlayer.GetInventory())
                {
                    ...
                    foreach (ItemDrop.ItemData item in items)
                    {
                        ...
                        if (item.IsBackpack())
                        {
                            if (!item.GetOrCreateBackpackComponent().GetInventory().IsTeleportable(allowAllItems))
                            {
