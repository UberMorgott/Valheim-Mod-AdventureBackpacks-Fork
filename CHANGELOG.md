...
* The support library vendored in 1.9.14.4 is gone. Everything it did is now the fork's own code on top of Jotunn and
  vanilla APIs: logging, config sync, recipes and drops (Jotunn `CustomRecipe`), per-item backpack data (vanilla
  `ItemData.m_customData`) and bone/shader handling for the backpack models. Backpack contents, prefab names, costs,
  drops and config keys are unchanged.
* API: `GetBackPackDefinitionFromComponent` returns the definition for backpack items instead of always failing.
* ConfigurationManager: the recipe cost and drop table editors are replaced by plain text fields using the same value
  format as the config file.
* Drop chances are parsed culture-independently, so `0.5` reads the same on a Russian or German system locale.
* The API stub bridge now resolves through `AssemblyResolve`; only the separate API stub DLL is supported.

## 1.9.14.4 - Morgott's fork: in-game ids renamed, support library vendored

* **Breaking for other mods and configs**: every in-game id the fork exposes was renamed from `vapok` to
  `adventurebackpacks`. Translation keys are now `$adventurebackpacks_item_*` (were `$vapok_mod_item_*`), status
  effects are `SE_adventurebackpacks_*` and the asset bundle is `adventurebackpacks`. Anything referring to the old
  names must be updated - for example shudnal's ExtraSlots `Items discovered` option, where
  `$vapok_mod_item_<name>` becomes `$adventurebackpacks_item_<name>`. Backpack prefab names, recipes, costs, drops
  and config keys are unchanged, and existing backpacks keep their contents.
* The bundled support library is now compiled into the plugin instead of being merged in at build time. Jotunn and
  vanilla APIs replaced the parts that duplicated them, and ILRepack was dropped, so the DLL is a single plain
  assembly again.
* Release builds are reproducible: the same commit always produces a byte-identical DLL (debug symbols are embedded
  rather than written next to it).
* Internal cleanups: analyzer and markdown lint warnings fixed, Unity project folders renamed to match the mod.

## 1.9.14.3 - Morgott's fork: container mod compatibility (Vapok 2.0.2 / 2.0.4 fixes)

* Mods that craft or build from nearby containers (Valheim Plus CraftFromChest, AzuCraftyBoxes) can no longer
  duplicate or reset the equipped backpack: the player's backpack container is never saved to or reloaded from the
  player ZDO.
* A backpack inventory change made through the container instance another mod holds is saved instead of dropped, and
  the container is kept pointed at the live backpack inventory.
* The backpack inventory is resized whenever its configured width or height changes, not only when it shrinks.
* Crafting/build requirement patches no longer depend on how the game stores the item count, so they keep working
  next to other mods that transpile the same methods.
...
* Fixed: InventoryGrid Index Error when opening a backpack up for the first time upon game load.
  ...
  * Additionally, swapping backpacks back and forth fixes the inventory grid issue.
...
* Changes
  * Updated Config Hint Text for Multiplier. Lot of confusion on this.
...
* Explorers Wisppack Wisplight is Now Toggleable
...
* Adds in Ukrainian Translation (Thanks to @Stadde1n for stopping by the Discord!)
...
* Configuration File is generated with split section names accidentally.  Now section names in the config file itself should all be in English, and Configuration Manager should show localized Section Names.
  * May or may not have an overall effect on random drops occurring.
...
* Arctic Backpack sizing was set to rugged on the X axis.  Oops.
...
* Resolves a Bag Duplication that was occurring when trying to insert a backpack into a backpack. (The gods are watching you.)

## 1.0.0.0

* Initial Release of Adventure Backpacks
  * This is a full refactor and completely re-written version of JotunnBackpacks.
